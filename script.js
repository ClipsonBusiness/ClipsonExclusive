document.addEventListener('DOMContentLoaded', () => {
    ...
    // Clipson checkout (Stripe Checkout Sessions; adds metadata)
    (function () {
        async function startCheckout() {
            const clipson = window.__clipson;
            const referralCode = clipson?.referralCode || '';
            const campaignId = clipson?.campaignId || '';

            const res = await fetch('/api/create-checkout', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ referralCode, campaignId }),
            });
        ...
        document.addEventListener('click', (e) => {
            const a = e.target?.closest?.('a[data-clipson-checkout="membership"]');
            if (!a) return;
            e.preventDefault();
            startCheckout().catch((err) => console.error(err));
        });
    })();
}); 